...
import { getCurrentTimeInTz, todayDateInTz, isValidTimeZone } from "./tz";
...
//
// Reuses the exported sendPushToUser and the shared tz helpers (./tz).
...
async function tick(): Promise<void> {
  ...
  for (const r of rows) {
    ...
    // The shared tz helpers fall back to UTC on an invalid zone; a reminder
    // fired at UTC time would be wrong (not just late), so skip outright rather
    // than nudge at the wrong local time. (Preserves the old {-1,-1}/"" skip.)
