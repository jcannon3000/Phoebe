 ...
 * numbered verses 4 at a time. Falls back to the original reference-
 * only "open your bible" slide when the local Bible JSON doesn't
 * carry the book (e.g. deuterocanonical readings).
 */
 ...
 * subtitle reads ("The Psalm Appointed For This Morning"). The
 * client used to derive this from the eyebrow string with a fragile
 * "FIRST" / "SECOND" / "GOSPEL" check; the server now stamps it
 * explicitly so the title card is unambiguous.
 */
...
export function buildLessonSlides(
  ...
  // Try to pull the actual verse text from local Bible data. Falls
  // back to a single reference-only slide when the book isn't
  // locally available (deuterocanon).
  const verses = lookupLessonVerses(trimmed);

  // Title slide either way — the user wanted the same big-headline
  // experience as the psalm_title slide, so even the fallback path
  // gets a title card before the body.
  ...
  if (!verses || verses.length === 0) {
    ...
    // lesson and a tap-out to Bible.com if they want to read it.
    return [
      titleSlide,
      {
        id: idGen(),
