...
const norm = (r) =>
  // A bracket leaves a separator on BOTH sides. Closing with nothing fused the
  // digits either side of it: "John 1:(29-34)35-42" became "1:, 29-3435-42",
  // which parseRef read as verse 3435. Collapsing the ":," it leaves keeps the
  // chapter's first verse rather than reading the whole chapter.
...
const THEME_PICKS = {
  ...
  // Owner: "use The birth of Jesus with shepherds, Jesus Mafa". The Sunday
  // after Christmas reads the Presentation (Luke 2:22-40) and nothing in the
  // library paints it; the Mafa nativity is the same chapter, so it shows
  // without claiming the verses. Id 48387 is the colour scan — 59366 is the
  // same painting cropped and in black and white.
  "Luke 2:22-40": 48387,
...
function build() {
  ...
  for (let i = 0; i < DAYS; i++) {
    ...
    // A pick that matches nothing fell back to art.refs[0] — which for the Good
    // Shepherd is "Psalm 23", the one caption this file promises never to give a
    // week, and which officePrefetch then saves as that week's reading. The
    // first NON-psalm reference is what a guess has always been captioned with.
    const ref = chosen.refOverride
      ?? (top > 0 ? art.refs.find((r) => matchScore([r], tierRefs) === top) : null)
      ?? (art.refs ?? []).find((r) => !isPsalmRef(r))
      ?? "";
    /**
