...
// Phoebe (web + iOS), not just on the native app. The native MusicKit plugin
// (CobreatheMusicPlugin) only exists in the iOS shell; on the web there's no
// Apple Music, so search has to go through the Apple Music API with a developer
// token. We sign that token server-side (ES256, MusicKit .p8 key) and proxy the
// search so the private key never reaches the client.
//
// Catalogue search needs ONLY the developer token — no user (Apple Music) auth.
// Playback of a result is separate: in-app via the native plugin on iOS, or a
// deep-link to music.apple.com everywhere else.
//
// Setup (one-time): Apple Developer portal → Certificates, IDs & Profiles →
// Keys → create a key with MusicKit enabled → download AuthKey_XXXX.p8. Then set
// these env vars (Railway):
//   APPLE_MUSIC_KEY_ID      — the 10-char Key ID
//   APPLE_MUSIC_TEAM_ID     — your 10-char Team ID
//   APPLE_MUSIC_PRIVATE_KEY — the .p8 contents (PEM; literal \n or base64 both ok)
