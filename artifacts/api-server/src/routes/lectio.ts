...
// ─── GET /api/lectio/:momentToken/:userToken/week/:sundayDate ───────────────
// Full read-only view of a specific (past or current) Sunday. All three stages
// are revealed with all members' reflections — this is the "Sunday gathering"
// view used both for today's read-only reveal and for browsing past weeks.

router.get("/lectio/:momentToken/:userToken/week/:sundayDate", async (req, res): Promise<void> => {
  const { momentToken, userToken, sundayDate } = req.params;
  if (!/^\d{4}-\d{2}-\d{2}$/.test(sundayDate)) {
    res.status(400).json({ error: "invalid_sunday_date" });
    return;
  }
  ...
  // Look up the reading for the requested Sunday.
  let reading: LectionaryReading;
  try {
    reading = await getReadingForSunday(new Date(sundayDate + "T12:00:00Z"));
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    reading = {
      id: 0,
  ...
  // All members + all reflections for the requested Sunday.
  const allMembers = await db
    .select()
    .from(momentUserTokensTable)
    .where(eq(momentUserTokensTable.momentId, moment.id));
  ...
  const reflections = await db
    ...
    .where(and(
      eq(lectioReflectionsTable.momentId, moment.id),
      eq(lectioReflectionsTable.sundayDate, reading.sundayDate),
    ));

  // Group reflections by stage, reveal them ALL (this is the communal view).
  const stageReveals: Record<Stage, {
    label: string;
    prompt: string;
    ...
    lectio:    { label: STAGE_LABELS.lectio,    prompt: STAGE_PROMPTS.lectio,    reflections: [], nonSubmitterNames: [] },
    meditatio: { label: STAGE_LABELS.meditatio, prompt: STAGE_PROMPTS.meditatio, reflections: [], nonSubmitterNames: [] },
    oratio:    { label: STAGE_LABELS.oratio,    prompt: STAGE_PROMPTS.oratio,    reflections: [], nonSubmitterNames: [] },
  };

  for (const s of STAGES) {
    const forStage = reflections
      .filter((r) => r.stage === s)
      .sort((a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime());
    stageReveals[s].reflections = forStage.map((r) => ({
      userName: r.userName,
      isYou: r.userToken === userToken,
      text: r.reflectionText,
      createdAt: r.createdAt.toISOString(),
    }));
    const submitterTokens = new Set(forStage.map((x) => x.userToken));
    stageReveals[s].nonSubmitterNames = allMembers
      .filter((m) => !submitterTokens.has(m.userToken))
      .map((m) => m.name ?? m.email.split("@")[0]);
  }
