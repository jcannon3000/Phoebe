// IconHowToIntro — a three-slide "how it works" for Praying with Icons, shown
// the first time someone enters the practice and reachable afterwards from the
// "How this works" pill on its opening screen.
//
...
const KEY = "phoebe:icon-howto-seen";

export function iconHowtoSeen(): boolean {
  try { return localStorage.getItem(KEY) === "1"; } catch { return false; }
}
export function markIconHowtoSeen(): void {
...
const SLIDES: Slide[] = [
  {
    eyebrow: "Praying with Icons",
    title: "One icon, one week",
    body: "You choose an icon and sit with it — the same one, every day, until Sunday. There is nothing to work out and nothing to finish. You look, and you let yourself be looked at.",
    art: "look",
  },
  {
    eyebrow: "Return to it",
    title: "Notice what arises",
    body: "Coming back to the same face across a week is the practice. What rises on Thursday is not what rose on Monday — a word, a memory, a resistance — and none of it needs to be resolved. It only needs to be noticed.",
    art: "week",
  },
  {
    // Last slide — the invitation, centred, no eyebrow and no headline.
    body: "Each week you can stay with the icon you have been sitting with, choose a new one, or take the one suggested for the coming Sunday's reading.",
    art: null,
  },
];
...
export function IconHowToIntro({ onDone, photos }: { onDone: () => void; photos?: string[] }) {
          ...
          */}
          ...
          >
            ...
            {isLast && (
              ...
              >
                {t("icons.howto_begin", { defaultValue: "Choose your icon" })} &rarr;
              </button>
            )}
          </motion.div>
      </div>
