/**
 * The Way of Love rule — a three-step flow: Listen → Pray → Learn.
 *
 * St. Benedict's Rule opens with "Listen," so the rule starts there: a few
...
export default function WayOfLoveRuleFlow({
  ...
  const commit = () => {
    ...
    // requests (pinned) → Listen (contemplation) → Pray (the office card) → the
    ...
    const order = ["requests", "contemplation", "office", newsletter, "feeds", "gratitude", "examen", "ncmp", "podcasts", ...others];
  ...
  if (step === "listen") {
    return shell(
      <>
        {backRow(onBack)}
        {stepHeader(1, t("wol_rule.listen_eyebrow", { defaultValue: "Listen" }), t("wol_rule.listen_title", { defaultValue: "Listen" }))}
        <p style={{ color: SAGE, fontSize: 15, fontFamily: FONT, lineHeight: 1.6, margin: "14px 0 0" }}>
          {t("wol_rule.listen_body", { defaultValue: "St. Benedict's Rule opens with “Listen.” Take a few minutes a day to sit in silence before God — listening for what God might be speaking, and what's on your own heart. Listening to God's love." })}
        </p>
  ...
  return shell(
    <div style={{ flex: 1, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", textAlign: "center" }}>
      ...
      <p style={{ color: SAGE, fontSize: 15, fontFamily: FONT, lineHeight: 1.6, margin: "10px 0 0", maxWidth: 340 }}>
        {t("wol_rule.done_sub", { defaultValue: "Your home now leads with Listen, your prayer, and your reflection. Return any time to change it." })}
      </p>
