...
export function DailyProgressBody({ showStreak = true, showDone, renderOfficeHero, leadCard, maxUpcoming, onRemainingCount, mountTag = "unlabeled" }: { showStreak?: boolean; showDone?: boolean; renderOfficeHero?: (side: "morning" | "evening") => ReactNode; leadCard?: ReactNode; maxUpcoming?: number; onRemainingCount?: (count: number) => void; /** Diagnostic only — see lib/celebrationDebugLog.ts. Identifies which of dashboard.tsx's mutually-exclusive render branches mounted this instance. */ mountTag?: string }) {
  ...
  /**
   * "ANDREW'S VERSION" — the weekly lectionary comment, on Taizé's terms.
   *
   * Owner: "create a weekly like taize version and make it admin only", and
   * "don't do a reader" — so tapping it opens the publication's own page as
   * they built it, not a stripped text column. Admin-only lives in
   * `andrewsActive` (useRhythmState), so this card simply never exists for a
   * reader.
   *
   * Marked read on OPEN, like every other card here that leaves for someone
   * else's site: the reader may finish there and never come back through the
   * app. And a card in Done still opens the post it just read, rather than
   * doing nothing — the fix its sibling needed.
   */
  const andrewsCard = {
    key: "andrews", emoji: "📰", rgb: "150,140,180",
    done: andrewsDone,
    href: "",
    onClick: () => {
   ...
   * separate creation prayer and contemplative prayer"). `contemplationStyle`
   * is only the aggregate; using it to decide what the MORNING card is meant a
  ...
  /**
   * THE CARDS FADE IN. THEY DO NOT RISE.
   *
   * Owner: "the borders shimmy up a little bit at the end of the animation …
   * it would be best if they just held still", and then, of the first attempt,
   * "they are still settling in wrong and end up looking weird."
   *
   * The border was never moving on its own — the card was. Animating `y` puts
   * the card on a GPU layer at fractional pixels; when it lands, the transform
   * stops being a transform, the layer is discarded, and the whole card
   * re-rasterises snapped to whole pixels. A hairline border straddling a half
   * pixel lands a fraction from where it sat a frame earlier. That is the
   * twitch, and it is worst on these cards precisely because of the thing the
   * card-border note already records: a frost layer that rounds outward over a
   * hairline at fractional offsets.
   *
   * The first fix pinned the layer with translateZ(0) so it would never be
   * discarded. It stopped the snap and cost more than it bought: a permanently
   * composited layer changes how the frost's blur is sampled underneath, which
   * is the "looking weird".
   *
   * So: remove the cause rather than pin it. With no transform there is no
   * fractional placement, no layer to discard and nothing to re-rasterise — the
   * card is drawn at its final position from the first frame and only its
