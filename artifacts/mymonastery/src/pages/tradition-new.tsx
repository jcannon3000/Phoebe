...
export default function TraditionNew() {
  ...
  async function handleCreate() {
    ...
    try {
      ...
      // A single organizer-set time — no alternates, no guest voting.
      const proposedTimes = [firstPick]
        .filter(Boolean)
      ...
      // Save the time + location → creates meetup + Google Calendar invite.
      // Location is per-meetup going forward, so it's sent here (not on the
      // ritual create). For a video-call gathering we send the meeting link
      // as the meetup location so the Google Calendar invite has a clickable
      // join link; in-app surfaces read the ritual-level meetingUrl. A single
      // time is set as confirmed immediately — no guest voting to wait on.
      await apiRequest("PATCH", `/api/rituals/${result.id}/proposed-times`, {
        proposedTimes,
        confirmedTime: proposedTimes[0],
