...
import { getGuestStepGoal, setGuestStepGoal } from "@/lib/guestSeed";
...
export default function DailyStepsPage() {
  ...
  // silence goal), not on the server. Signed-in users use office-prefs.
  const { isGuest } = useGuestMode();
  const [guestGoal, setGuestGoal] = useState<number>(() => getGuestStepGoal());
  ...
  const goal = isGuest ? guestGoal : (prefs?.dailyStepGoal ?? 0);
  ...
  const applyGoal = (g: number) => {
    if (isGuest) { setGuestStepGoal(g); setGuestGoal(g); }
    else { setGoal.mutate(g); }
  };
