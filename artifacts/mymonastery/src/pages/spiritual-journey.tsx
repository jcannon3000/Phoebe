// ─── The Spiritual Journey — a Coursera-style video course (web only) ─────────
//
// A guided walk through Fr. Thomas Keating's "Spiritual Journey" series. Videos
// play in-app via the YouTube IFrame API (so we can detect when one ends and
// offer the next), can go fullscreen, and completion is tracked device-locally.
// This page is intentionally web-only — see the isNativeShell() gate below.
...
// Frosted-glass treatment shared by the course cards.
const FROST = {
...
export default function SpiritualJourneyPage() {
  const { completedCount, isComplete, toggleComplete, markComplete, setLast, lastId } = useCourseProgress(SPIRITUAL_JOURNEY.id);
  ...
  const [activeId, setActiveId] = useState<string>(() => {
    try {
      ...
      if (q && getVideo(q)) return q;
    } catch { /* ignore */ }
    if (lastId && getVideo(lastId)) return lastId;
  ...
  const active = getVideo(activeId);
  const next = getVideo(activeId) ? nextVideo(activeId) : undefined;
  const prev = getVideo(activeId) ? prevVideo(activeId) : undefined;
  ...
  const pct = Math.round((completedCount / JOURNEY_TOTAL) * 100);
      ...
      <Layout bgPhoto={leafBg}>
        <div className="mx-auto w-full max-w-md px-2 py-16 text-center">
          ...
          <p className="mt-3 text-sm leading-relaxed" style={{ color: C.sage }}>
            ...
            <span style={{ color: C.dim }}>withphoebe.app/journey</span> in your browser to watch
            Fr. Thomas Keating's series and track your progress.
          </p>
          <Link
            href="/menu/practices"
        ...
        <div className="mb-5">
          ...
          <div className="mt-4">
            <div className="mb-1.5 flex items-center justify-between text-[11px]" style={{ color: C.sage }}>
              <span>{completedCount} of {JOURNEY_TOTAL} lessons complete</span>
              <span>{pct}%</span>
            </div>
        ...
        <div className="mt-5 flex flex-col gap-6 lg:flex-row">
            ...
            {active && (
              <div className="mt-4">
                ...
                <p className="mt-1 text-[11px]" style={{ color: "rgba(143,175,150,0.6)" }}>
                  Lesson {active.index} of {JOURNEY_TOTAL} · playing inside Phoebe — tap ⛶ for fullscreen
                </p>
          ...
          <div className="lg:w-[360px] lg:shrink-0">
            <p className="mb-2 px-1 text-[11px] font-semibold uppercase tracking-widest" style={{ color: "rgba(143,175,150,0.7)" }}>
              Course contents · {SPIRITUAL_JOURNEY.units.length} units
            </p>
            <div className="space-y-2">
              {SPIRITUAL_JOURNEY.units.map((unit) => (
                <UnitBlock
                  key={unit.id}
                  unit={unit}
                  activeId={activeId}
