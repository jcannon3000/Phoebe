...
export default function GuidedPrayerPage() {
  ...
  /**
   * THE DEAN'S COMMENTARY OFFER IS GONE (owner, 2026-09-14: "take out the
   * last slide of simple guided when someone is subscribed to the Dean's
   * Commentary where it invites them to read it"). It was the last slide for
   * a reader who kept VTS and hadn't read today's; the reading now waits on
   * its own home card, and the prayer ends where its shape ends. Like
   * showCollect, the constant keeps the tail arithmetic honest rather than
  ...
  /** A picture that couldn't come — or Visio Divina switched off — is stepped over, both ways. */
  const skipped = (s: number) => isPictureStep(s) && (!visioOn || pictureState[s / 2 - 1] === "failed");
    ...
    >
      ...
      <main
        ...
        style={{
          ...
          // BALANCED: the column centres in the space BETWEEN the header and
          // the bottom control band, with the same breathing room on each
          // side (owner: "make sure the content is vertically centered and
          // balanced"). It used to centre between the screen's top edge and a
          // tall bottom reserve, which sat every slide visibly high.
          paddingTop: "calc(max(1.25rem, calc(var(--safe-top) + 0.5rem)) + 64px)",
      ...
      >
        <AnimatePresence mode="wait">
          ...
          {pictureOf && (
            ...
            >
              {/* The movement as an eyebrow, the picture, then the work itself —
                  its title where the movement's name used to stand and its
                  credit right beneath (owner: "Put the credits for each picture
                  on their slide", "Have the title of the image instead of just
                  suplication"). No frame round the painting.

                  The box takes the work's measured shape before the image
                  arrives, so a late picture fades into place instead of moving
                  the words. The height allowance is the column (header and
                  bottom-band clearance — see <main>) less the eyebrow, a
                  two-line title, the artist line and a three-line credit, so the
                  whole stack stays centred on every phone. */}
              ...
              {pictureArt && (
                <>
                  <h2 style={{ color: WARM, fontFamily: FONT, fontWeight: 600, fontSize: "clamp(18px, 4.8vw, 21px)", lineHeight: 1.25, letterSpacing: "-0.005em", margin: "16px 0 0" }}>
                    {pictureArt.title}
                  </h2>
                  {(pictureArt.artist || pictureArt.date) && (
                    <p style={{ color: "rgba(240,237,230,0.72)", fontFamily: FONT, fontSize: 13.5, lineHeight: 1.4, margin: "6px 0 0" }}>
          ...
          {/* (The Dean's Commentary offer went with its slide — see showDeanOffer.) */}
        </AnimatePresence>
      </main>
