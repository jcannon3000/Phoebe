...
const body = rows
  .map(([ymd, r]) =>
    ...
    `psalm: ${JSON.stringify(r.psalm)}, ot: ${JSON.stringify(r.ot)}, url: ${JSON.stringify(r.url)} },`)
  .join("\n");
...
export type RclSunday = {
  ...
  nt: string[];
  psalm: string | null;
  /** The lectionarypage.net page this came from. Kept because its PATH encodes
   *  the liturgical day — YearA_RCL/Pentecost/AProp17_RCL — which is how a work
   *  tagged "Year A Proper 17th Sunday" can be matched to the Sunday without a
   *  second calendar. */
  url: string;
